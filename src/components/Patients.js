        ...
        <div className='patients-container'>
            ...
            {data.map((patient, index) => (
                <div key={index} className="patient-card">
                    <img src="../assets/userLogo.jpg" alt={patient.name} className="patient-image" />
                    <div className="patient-details">
                        <h3 className="patient-name">{patient.username}</h3>
                        <p className="patient-date">Member Since: {formatDate(patient.created_at)}</p>
                    </div>
                        <button 
                            className="leave-patient-button" 
                            onClick={() => {
                                if (window.confirm("Are you sure you want to leave this patient?")) {
                                    axios.delete(`http://localhost:5000/api/responsibles/${patient.user_id}`)
                                        .then(() => {
